...
def build_qc_report(
    ...
    lines = [
        ...
        f"- square_strategy_used: {summary.get('square_strategy_used')}",
        f"- adhesion_strategy_used: {summary.get('adhesion_strategy_used')}",
        f"- accepted_square_instances: {summary.get('n_accepted_square_instances')}",
        f"- accepted_cluster_split_instances: {summary.get('n_accepted_cluster_split_instances')}",
        "",
        "Manual QC checklist",
        ...
        "- Check 02c_square_preflight_overlay.png when square_strategy_used is true.",
        "- Check 07d_square_contour_refined_overlay.png for square/rectangular crystal boundaries.",
        "- Check 07g_cluster_split_overlay.png for over-splitting of lightly adhered crystals.",
    ...
    lines.extend(
        [
            ...
            "- Square-like contour candidates are gated and used only as localization/refinement seeds.",
            "- Cluster splitting is conservative and should be manually reviewed when present.",
